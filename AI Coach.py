...
if 'duration_seconds' not in st.session_state:
    st.session_state.duration_seconds = 120  # default 2 دقيقة
...
def generate_feedback(audio_res, vision_res):
    ...
    if arms >= 75:
        ...
        improvements.append("👐 كنت بتقفل ذراعيك أحياناً وده ممكن يديك مظهر دفاعي.")
...
# ─── إعداد وقت العرض (بيظهر بس لما مش بيسجل ومش التقرير جاهز)
if not st.session_state.is_recording and not st.session_state.report_ready:
    ...
    # نحفظ الوقت في session_state عشان start_recording تلاقيه
    st.session_state.duration_seconds = max(10, total_seconds)  # minimum 10 ثواني
...
with col1:
    st.button("🟢 بدء التقييم",
              on_click=start_recording,
              disabled=st.session_state.is_recording,
              use_container_width=True)
with col2:
    st.button("🔴 إنهاء مبكر",
...
# ─── تشغيل الـ Threads + Timer
if st.session_state.is_recording and not st.session_state.threads_started:

    def run_audio():
        result = analyze_presentation_audio(_state['stop_event'])
        _state['audio_results'] = result

    def run_vision():
        result = analyze_presentation_vision(_state['stop_event'])
        _state['vision_results'] = result
...
if st.session_state.is_recording:
    ...
    # الوقت خلص → وقف التسجيل أوتوماتيك
    if st.session_state.is_recording:
        st.session_state.is_recording = False
        _state['stop_event'].set()
        st.rerun()

# ─── الانتظار بعد الإيقاف
