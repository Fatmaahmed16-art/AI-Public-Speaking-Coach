...
def analyze_presentation_vision(stop_event):
    """
    👁️🎬 موديول الرؤية المدمج الشامل
    يقوم بفتح الكاميرا وتحليل: تعبيرات الوجه، الرمش، هزة الرأس،
    التواصل البصري، وضعية الأكتاف، وحركة الأيدي.
    """

    # ✅ إصلاح: التحقق من وجود ملف الموديل قبل التشغيل
    model_path = "face_landmarker.task"
    if not __import__("os").path.exists(model_path):
        print(fix_arabic(f"🔴 خطأ: ملف الموديل '{model_path}' غير موجود!"))
        return {
            "eye_contact_pct": 0.0,
            "face_confident_pct": 0.0,
            "face_neutral_pct": 0.0,
            "face_tense_pct": 0.0,
            "posture_score": 0.0,
            "arms_open_pct": 0.0,
            "movement_intensity": "غير متوفر (ملف الموديل مفقود)",
    ...
    # ✅ إصلاح: التحقق إن الكاميرا اتفتحت فعلاً
    if not cap.isOpened():
        print(fix_arabic("🔴 خطأ: مش قادر يفتح الكاميرا!"))
        return {
            "eye_contact_pct": 0.0,
            "face_confident_pct": 0.0,
            "face_neutral_pct": 0.0,
            ...
            "posture_score": 0.0,
            "arms_open_pct": 0.0,
            "movement_intensity": "غير متوفر (الكاميرا مغلقة)",
            "total_vision_time": 0.0,
        }
    ...
    while cap.isOpened() and not stop_event.is_set():
        ...
        if detection_results.face_landmarks:
            ...
            if face_size == 0:
                face_size = 0.001  # ✅ إصلاح: تجنب القسمة على صفر
            ...
            if prev_nose_x is not None:
                move_dist = (
                    np.sqrt((nose_x - prev_nose_x) ** 2 + (nose_y - prev_nose_y) ** 2)
                    / face_size
                )
            ...
            elif blink_rate > 75.0 or head_movement_score > 45.0:
        ...
        if lmList and len(lmList) > 16:
            ...
            if (shoulder_diff / shoulder_width) > 0.06:
                bad_posture_duration += frame_delta
            ...
            if wrist_to_elbow1 < (shoulder_width * 0.5) and wrist_to_elbow2 < (shoulder_width * 0.5):
                crossed_arms_duration += frame_delta
                arms_status = "Arms : Crossed (Defensive)"
            else:
                arms_status = "Arms : Open & Welcoming"

        # ✅ إصلاح: إضافة movement_samples في كل frame مش في الآخر بس
        movement_samples.append(head_movement_score)
        ...
        cv2.putText(frame, f"Face: {face_status}", (20, 90), cv2.FONT_HERSHEY_COMPLEX, 0.45, color_face, 1)
        cv2.putText(frame, f"Blink Rate: {blink_rate:.1f} RPM", (20, 110), cv2.FONT_HERSHEY_COMPLEX, 0.45, (200, 200, 200), 1)

        cv2.imshow("AI Stage Coach - Vision Module", frame)
    ...
    if total_valid_frame == 0 or total_time == 0:
        return {
            "eye_contact_pct": 0.0,
            "face_confident_pct": 0.0,
            "face_neutral_pct": 0.0,
    ...
    avg_movement = sum(movement_samples) / len(movement_samples) if movement_samples else 0
    movement_str = "(فرط حركة) عالية" if avg_movement > 20 else "هادئة ومستقرة"

    eye_contact_pct = (focused_frames / total_valid_frame * 100) if total_valid_frame > 0 else 0.0

    # ✅ إصلاح المشكلة الرئيسية: كانت الأقواس غلط فكانت بتجمع neutral_duration / total_time بس
    confident_pct = ((confident_duration + neutral_duration) / total_time * 100) if total_time > 0 else 0.0
    ...
    return {
        ...
        "posture_score": round(100 - (bad_posture_duration / total_time * 100), 1),
        "arms_open_pct": round(100 - (crossed_arms_duration / total_time * 100), 1),
        "movement_intensity": movement_str,
